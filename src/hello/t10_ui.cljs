...
(defn input-field [{:keys [label value on-change invalid?]}]
  [:div
   [:label label]
   [:input {:value value
            :on-change on-change
            :style {:border (when invalid? "1px solid #ff0000")}}]])

;; This is how to render a component into the DOM
(r/render [button {:on-click #(js/alert "hello!")} "alert"]
          (gdom/getElement "button-target"))
...
(defn ajax-subscribe-form []
  [:div.form
   ...
   [button {:on-click #(js/alert @email)} "Subscribe"]])
...
(defn ajax-subscribe-form-with-state []
  (let [email (r/atom "")]
    (fn []
      (let [value @email
            short? (> (count value) 3)
            not-email? (not (re-matches #".+@.+\..+" value))
            invalid? (and short? not-email?)
            disabled? not-email?]
        [:div.form
         [input-field {:label "email (with validation)"
                       :value value
                       :invalid? invalid?
                       :on-change #(reset! email (.. % -target -value))}]
         [button {:on-click #(js/alert value)
                  :disabled? disabled?}
          "Subscribe"]]))))
