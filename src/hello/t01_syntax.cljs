...
;; Idiomatic Clojure naming is to use kebab case:
...
;; Sometimes, side-effecting functions are suffixed with `!` character
...
;; In fact `defn` is a macro, which expands into `def` + `fn`
...
;; note that `macroexpand` takes a form as data (quoted with ')
...
;; To make it more convenient Clojure has a shorthand notation for lambdas (anonymous functions).
...
;; This form can be written down using shorthand syntax

(map #(+ % 10) [0 1 2 3])

;; Shorthand syntax consists of a function body prepended with # character.
...
;; Nesting lambdas in a shorthand notation is not allowed.
